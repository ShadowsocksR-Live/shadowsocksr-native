...
static struct server_t *new_server(int fd, int method);
...
server_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    ssize_t r = recv(server->fd, remote->buf->buffer, BUF_SIZE, 0);
        ...
        if (r == 0) {
            ...
            if (cork_ip_init(&ip, sa->host) != -1) {
                if (ip.version == 4) {
                    ...
                    abuf->buffer[abuf->len++] = 1;
                    memcpy(abuf->buffer + abuf->len, &host, host_len);
                    abuf->len += host_len;
                ...
                abuf->buffer[abuf->len++] = host_len;
                memcpy(abuf->buffer + abuf->len, sa->host, host_len);
                abuf->len += host_len;
            }
...
accept_cb(EV_P_ ev_io *w, int revents)
{
    ...
    struct server_t *server = new_server(serverfd, listener->method);
    struct remote_t *remote = new_remote(remotefd, listener->timeout);
...
main(int argc, char **argv)
{
    ...
    memset(&listen_ctx, 0, sizeof(struct listen_ctx_t));
